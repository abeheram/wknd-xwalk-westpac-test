export default function decorate(block) {
  const pageHeader = block.querySelector('.page-header');  // Select the page-header div
  if (pageHeader) {
    // Remove the unwanted <div><div></div></div> structure
    const unwantedDiv = pageHeader.querySelector("div > div");
    if (unwantedDiv) {
      unwantedDiv.parentElement.remove(); // Remove the parent <div> containing the unwanted structure
    }
